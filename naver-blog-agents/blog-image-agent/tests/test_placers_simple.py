"""
AutoPlacer 및 HtmlInserter 간단한 테스트
"""
import sys
...
# 직접 import
from placers.auto_placer import AutoPlacer
from placers.html_inserter import HtmlInserter
...
"""
    ...
    assert len(positions) == 3, "3개의 위치가 발견되어야 함"
    assert positions[0].section_title == "첫 번째 섹션", "첫 번째 섹션 제목 확인"
    print("\n[OK] 테스트 통과")
...
"""
    ...
    assert len(placements) >= 1, "최소 1개 이상의 배치가 있어야 함"
    assert placements[0].alt_text, "Alt text가 생성되어야 함"
    print("\n[OK] 테스트 통과")
...
def test_html_inserter_basic():
    ...
    assert "img" in image_html, "img 태그가 있어야 함"
    assert "alt=" in image_html, "alt 속성이 있어야 함"
    assert "loading=" in image_html, "loading 속성이 있어야 함"
    print("\n[OK] 테스트 통과")
...
def test_html_inserter_insertion():
    ...
    assert "figure" in result, "figure 태그가 있어야 함"
    assert "image1.jpg" in result, "이미지 파일명이 있어야 함"
    print("\n[OK] 테스트 통과")
...
def test_html_inserter_gallery():
    ...
    assert "image-gallery" in gallery_html, "갤러리 클래스가 있어야 함"
    assert gallery_html.count("<figure") == 4, "4개의 figure가 있어야 함"
    print("\n[OK] 테스트 통과")
...
'''
    ...
    assert len(warnings) > 0, "alt 없는 이미지에 대한 경고가 있어야 함"
    print("\n[OK] 테스트 통과")
...
def test_integration():
    ...
    assert stats['total_images'] >= 1, "이미지가 삽입되어야 함"
    assert stats['with_alt'] >= 1, "Alt 텍스트가 있어야 함"
    print("\n[OK] 테스트 통과")
